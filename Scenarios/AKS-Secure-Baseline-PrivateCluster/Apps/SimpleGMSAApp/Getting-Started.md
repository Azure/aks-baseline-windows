...
## Note on the resource contraints in the workload manifest
The values used for CPU and memory represent the minimums to run a Windows application with GMSA. Windows applications require higher memory thresholds than Linux applications. You may need to adjust these values for running your own application on this reference framework. 
